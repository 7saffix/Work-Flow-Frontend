...
export default function Suppliers() {
  ...
  useEffect(() => {
    ...
    const fetchSuppliers = async () => {
      ...
      try {
        ...
        dispatch(setSuppliersData(res.data?.data || res.data || []));
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await api.post("/supplier/create", formData);
      if (res.data?.success || res.status === 201 || res.status === 200) {
        setFormData(initialFormState);
        setIsOpen(false);
        // Refresh directory instantly
        const updatedList = await api.get("/supplier");
        dispatch(setSuppliersData(updatedList.data?.data));
      }
    } catch (err) {
      console.error("Supplier addition failed:", err);
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="overflow-x-auto">
          {suppliers.length === 0 ? (
            <div className="p-12 text-center text-slate-400 font-medium text-sm">
              No suppliers registered in index directory.
            </div>
          ) : (
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-brand-900/20 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-md rounded-[24px] sm:rounded-[32px] border border-slate-100 shadow-2xl overflow-hidden relative">
            {/* Modal Header */}
            <div className="px-5 py-5  sm:px-8 sm:py-4 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
              <div>
            ...
            {/* Modal Form Content */}
            <form onSubmit={handleSubmit} className="px-5 sm:px-8 space-y-3">
              ...
              <div className="flex gap-3 p-4 border-t border-slate-50">
                <button
                  type="button"
                  onClick={() => {
                    setFormData(initialFormState);
